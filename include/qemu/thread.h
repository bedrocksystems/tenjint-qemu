...
typedef void (*QemuCondWaitFunc)(QemuCond *c, QemuMutex *m, const char *f,
                                 int l);
typedef int (*QemuCondTimedWaitFunc)(QemuCond *c, QemuMutex *m, time_t secs,
                                      const char *f, int l);
...
#define qemu_cond_timedwait(c, m, s)                                    \
            qemu_cond_timedwait_impl(c, m, s, __FILE__, __LINE__);
#else
#define qemu_mutex_lock(m) ({                                           \
...
int qemu_cond_timedwait_impl(QemuCond *cond, QemuMutex *mutex, time_t secs,
                             const char *file, const int line);
...
static inline int (qemu_cond_timedwait)(QemuCond *cond, QemuMutex *mutex, time_t secs)
{
    return qemu_cond_timedwait(cond, mutex, secs);
}
